...
if "ultime_domande" in st.session_state:
    for i, domanda in enumerate(st.session_state["ultime_domande"], start=1):
        st.markdown("### Domanda {}".format(i))
        st.write(domanda["domanda"])
        for j, opzione in enumerate(domanda["opzioni"]):
            st.write("{}. {}".format(chr(65 + j), opzione))
        st.success("✅ Risposta corretta: {}".format(domanda["corretta"]))
        st.info("📘 Spiegazione dettagliata: {}".format(domanda["spiegazione"]))
        st.warning("🎯 Distrattore comune: {}".format(domanda["distrattore"]))
        st.markdown("---")
